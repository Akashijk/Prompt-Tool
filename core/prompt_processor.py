...
from .template_engine import PromptSegment
from .csv_manager import CSVManager
from .config import config, DEFAULT_ENHANCEMENT_INSTRUCTION, DEFAULT_VARIATION_INSTRUCTIONS
...
class PromptProcessor:
    ...
    def initialize(self) -> None:
        ...
        self.template_engine.load_wildcards()
        
        self._update_status("Loading templates...")
        self.template_engine.list_templates()
        
        self._update_status("Initializing system prompts...")
        self._initialize_system_prompts()
        
        self._update_status("Ready")
    
    def _initialize_system_prompts(self):
        """Ensures system prompt files exist, creating them from defaults if necessary."""
        os.makedirs(config.SYSTEM_PROMPT_DIR, exist_ok=True)

        # Enhancement prompt
        enhancement_path = os.path.join(config.SYSTEM_PROMPT_DIR, 'enhancement.txt')
        if not os.path.exists(enhancement_path):
            with open(enhancement_path, 'w', encoding='utf-8') as f:
                f.write(DEFAULT_ENHANCEMENT_INSTRUCTION)

        # Variation prompts
        for key, content in DEFAULT_VARIATION_INSTRUCTIONS.items():
            var_path = os.path.join(config.SYSTEM_PROMPT_DIR, f'{key}.txt')
    ...
    def get_wildcard_files(self) -> List[str]:
        ...
        return self.template_engine.list_wildcard_files()

    def load_wildcard_content(self, wildcard_file: str) -> str:
        """Load content of a single wildcard file."""
        return self.template_engine.load_wildcard_content(wildcard_file)

    def save_wildcard_content(self, wildcard_file: str, content: str) -> None:
        """Save content to a wildcard file."""
        self.template_engine.save_wildcard_content(wildcard_file, content)
    
    def load_template_content(self, template_file: str) -> str:
        """Load template content."""
        return self.template_engine.load_template(template_file)

    def save_template_content(self, template_file: str, content: str) -> None:
        """Saves modified template content back to its file."""
        self.template_engine.save_template(template_file, content)

    def archive_template(self, template_file: str) -> None:
        """Archives a template file."""
        self.template_engine.archive_template(template_file)

    def archive_wildcard(self, wildcard_file: str) -> None:
        """Archives a wildcard file."""
        self.template_engine.archive_wildcard(wildcard_file)
    ...
    def get_system_prompt_files(self) -> List[str]:
        ...
        return sorted([f for f in os.listdir(config.SYSTEM_PROMPT_DIR) if f.endswith('.txt')])

    def load_system_prompt_content(self, filename: str) -> str:
        """Load the content of a system prompt file."""
        filepath = os.path.join(config.SYSTEM_PROMPT_DIR, filename)
        try:
    ...
    def get_default_system_prompt(self, filename: str) -> str:
        ...
        if key == 'enhancement':
            return DEFAULT_ENHANCEMENT_INSTRUCTION
        return DEFAULT_VARIATION_INSTRUCTIONS.get(key, '')
